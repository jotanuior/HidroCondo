...
async function canOperateCondo(req: AuthenticatedRequest, condominiumId: string, accountId: string | null) {
  if (isSuper(req)) return true;
  if (!['admin','sindico'].includes(req.auth?.role ?? '')) return false;
  const q = await pool.query(`SELECT 1 WHERE
       ($3::uuid IS NOT NULL AND $3::uuid IN(SELECT account_id FROM account_members WHERE user_id=$1 AND role IN('admin','sindico')))
    OR ($3::uuid IS NOT NULL AND $3::uuid IN(SELECT scope_id FROM access_grants WHERE user_id=$1 AND scope_type='account' AND role IN('admin','sindico')))
    OR $2::uuid IN(SELECT condominium_id FROM user_condominiums WHERE user_id=$1)
    OR $2::uuid IN(SELECT scope_id FROM access_grants WHERE user_id=$1 AND scope_type='condominium' AND role IN('admin','sindico'))`,
    [req.auth!.sub, condominiumId, accountId]
  );
  return q.rowCount === 1;
}

async function canReadSensor(req: AuthenticatedRequest, sensorId: string) {
  if (isSuper(req)) return true;
  const q = await pool.query(`SELECT 1
     ...
     WHERE s.id=$2 AND (
           s.account_id IN(SELECT account_id FROM account_members WHERE user_id=$1)
        OR s.account_id IN(SELECT scope_id FROM access_grants WHERE user_id=$1 AND scope_type='account')
        OR c.id IN(SELECT condominium_id FROM user_condominiums WHERE user_id=$1)
        OR c.id IN(SELECT scope_id FROM access_grants WHERE user_id=$1 AND scope_type='condominium')
        OR b.id IN(SELECT scope_id FROM access_grants WHERE user_id=$1 AND scope_type='building')
        OR u.id IN(SELECT scope_id FROM access_grants WHERE user_id=$1 AND scope_type='unit')
        OR s.id IN(SELECT scope_id FROM access_grants WHERE user_id=$1 AND scope_type='sensor')
     )`, [req.auth!.sub,sensorId]);
  return q.rowCount === 1;
}
...
export function registerAccountRoutes(app: Express) {
  app.post('/api/v1/condominios', requireAuth, async (req: AuthenticatedRequest, res, next) => {
    ...
    try {
      ...
      await client.query('INSERT INTO user_condominiums(user_id,condominium_id) VALUES($1,$2) ON CONFLICT DO NOTHING',[req.auth!.sub, condo.id]);
      await client.query(`INSERT INTO access_grants(user_id,scope_type,scope_id,role,created_by)
         VALUES($1,'condominium',$2,'admin',$1) ON CONFLICT DO NOTHING`,[req.auth!.sub, condo.id]);
      await client.query(`INSERT INTO audit_log(user_id,action,entity_type,entity_id,payload)
         VALUES($1,'create','condominium',$2,$3::jsonb)`,[req.auth!.sub, condo.id, JSON.stringify(p)]);
  ...
  app.post('/api/v1/sensores/claim', requireAuth, async (req: AuthenticatedRequest, res) => {
    ...
    let scae;
    try { scae = await validateSerialInScae(parsed.data); }
    catch (error) { return res.status(502).json({ error: error instanceof Error ? error.message : 'Falha ao validar sensor no SCAE' }); }
    ...
    try {
      ...
      let sensor;
      if (found.rowCount) {
        const q = await client.query(`UPDATE sensors SET account_id=$1,claimed_by=COALESCE(claimed_by,$2),claimed_at=COALESCE(claimed_at,now()),sensor_type='09',active=true,
             central_serial=COALESCE(central_serial,$3) WHERE id=$4 RETURNING *`,[account.id, req.auth!.sub, scae.central_serial || null, found.rows[0].id]);
        sensor = q.rows[0];
      } else {
        const q = await client.query(`INSERT INTO sensors(serial,sensor_type,central_serial,account_id,claimed_by,claimed_at,active)
           VALUES($1,'09',$2,$3,$4,now(),true) RETURNING *`,[parsed.data, scae.central_serial || null, account.id, req.auth!.sub]);
        sensor = q.rows[0];
      }
      await client.query(`INSERT INTO audit_log(user_id,action,entity_type,entity_id,payload)
         VALUES($1,'claim','sensor',$2,$3::jsonb)`,[req.auth!.sub, sensor.id, JSON.stringify({ serial: parsed.data, account_id: account.id })]);
      await client.query('COMMIT');
  ...
  app.post('/api/v1/sensores/:id/instalar', requireAuth, async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      if (active.rowCount) await client.query(`UPDATE sensor_installations SET removed_at=now(),removed_by=$1,reason=COALESCE($2,reason) WHERE id=$3`,[req.auth!.sub,parsed.data.reason||'Transferência de unidade',active.rows[0].id]);
      const installation = await client.query(`INSERT INTO sensor_installations(sensor_id,unit_id,installed_by,reason)
         VALUES($1,$2,$3,$4) RETURNING *`,[sensorId,parsed.data.unit_id,req.auth!.sub,parsed.data.reason||null]);
      await client.query('UPDATE sensors SET unit_id=$1,active=true WHERE id=$2',[parsed.data.unit_id,sensorId]);
      await client.query(`INSERT INTO audit_log(user_id,action,entity_type,entity_id,payload)
         VALUES($1,'install','sensor',$2,$3::jsonb)`,[req.auth!.sub,sensorId,JSON.stringify({unit_id:parsed.data.unit_id,reason:parsed.data.reason||null})]);
      await client.query('COMMIT');
  ...
  app.post('/api/v1/sensores/:id/remover', requireAuth, async (req: AuthenticatedRequest, res) => {
    ...
    const sensor = await pool.query(`SELECT s.id,s.account_id,c.id condominium_id FROM sensors s
    ...
    if (!row.condominium_id || !await canOperateCondo(req,row.condominium_id,row.account_id)) return forbidden(res);
    ...
    try {
      ...
      await client.query(`INSERT INTO audit_log(user_id,action,entity_type,entity_id,payload)
         VALUES($1,'remove_installation','sensor',$2,$3::jsonb)`,[req.auth!.sub,sensorId,JSON.stringify({reason})]);
      await client.query('COMMIT');
  ...
  app.get('/api/v1/sensores/:id/instalacoes', requireAuth, async (req: AuthenticatedRequest, res) => {
    ...
    if (!uuid.safeParse(sensorId).success) return res.status(400).json({ error: 'Sensor inválido' });
    if (!await canReadSensor(req,sensorId)) return forbidden(res);
    const q = await pool.query(`SELECT si.id,si.sensor_id,si.unit_id,si.installed_at,si.removed_at,si.reason,
