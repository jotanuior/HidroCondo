...
function canAdjust(req: AuthenticatedRequest) {
  return ['superadmin', 'admin', 'sindico'].includes(req.auth?.role ?? '');
}
...
async function sensorAccess(req: AuthenticatedRequest, sensorId: string) {
  const result = await pool.query(
    `SELECT s.id, s.serial, s.virtual_counter, s.last_raw_value, s.unit_id,
            c.id AS condominium_id, c.name AS condominium_name,
            b.name AS building_name, u.identifier AS unit_identifier
       FROM sensors s
  ...
  if (!sensor.condominium_id) return { sensor, allowed: false };
  const access = await pool.query(
    'SELECT 1 FROM user_condominiums WHERE user_id=$1 AND condominium_id=$2',
    [req.auth!.sub, sensor.condominium_id]
  );
...
export function registerCounterRoutes(app: Express) {
  app.post('/api/v1/sensores/:id/leitura-hidrometro', requireAuth, async (req: AuthenticatedRequest, res) => {
    if (!canAdjust(req)) return forbidden(res);
    ...
    try {
      ...
      await client.query(
        `INSERT INTO audit_log(user_id,action,entity_type,entity_id,payload)
         VALUES($1,'meter_reading_adjustment','sensor',$2,$3::jsonb)`,
        [
          req.auth!.sub,
          sensor.id,
          JSON.stringify({
            serial: sensor.serial,
            condominium_id: sensor.condominium_id,
            condominium_name: sensor.condominium_name,
            building_name: sensor.building_name,
            unit_identifier: sensor.unit_identifier,
            previous_reading_m3: previous,
            new_reading_m3: next,
            raw_sensor_value: sensor.last_raw_value,
  ...
  app.get('/api/v1/sensores/:id/ajustes-leitura', requireAuth, async (req: AuthenticatedRequest, res) => {
    ...
    if (!allowed) return forbidden(res);
    const result = await pool.query(
      `SELECT a.id,a.created_at,a.payload,u.name AS user_name,u.email AS user_email
         FROM audit_log a
